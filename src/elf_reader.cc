...
bool elf_reader::load_segment(void) {
    ...
    size_t align = get_max_aligment((void*)m_phdr, m_phdr_num);    
    ...
    for (size_t i = 0; i < m_phdr_num; ++i) {
        ...
        if (seg_addr == MAP_FAILED) {
            log_error("couldn't map \"%s\" segment %zd: %s\n", m_soname.c_str(), i, strerror(errno));
            return false;
        }
    }
    return true;
...
void elf_reader::dump_program_headers(void) {
    ...
    if (get_elf_class() == ELFCLASS64) {
        ...
        for (int i = 0; i < this->m_phdr_num; i++) {
            ...
            log_info("[%.2d]        %-12s  0x%0.8lx  0x%0.16lx  0x%0.16lx  %0.8lx  %0.8lx  %0.4x\n",
                     i,
        ...
        for (int i = 0; i < this->m_phdr_num; i++) {
            ...
            log_info("[%.2d]        %-12s  0x%0.8lx  0x%0.8lx  0x%0.8lx  %0.8lx  %0.8lx  %0.4x\n",
                     i,
                     elf_phdr_type_name((int)(phdr[i].p_type)),
                     (unsigned long)phdr[i].p_offset,
                     (unsigned long)phdr[i].p_vaddr,
